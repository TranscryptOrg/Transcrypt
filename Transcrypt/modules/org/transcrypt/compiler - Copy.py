...
class Generator (ast.NodeVisitor):
	...
	def visit_arguments (self, node):	# Visited for def's, not for calls
		...
		# Defaults for keyword args unconditionally, since they will be passed only after this point, using a __kwargsdict__
		...
		self.emit ('if (type (arguments [{}]) == __kwargdict__) {{\n', self.getTemp ('ilastarg'))
		self.indent ()
		...
		if node.vararg or node.kwarg or node.kwonlyargs:		
			...
			if node.kwarg or node.kwonlyargs:			
