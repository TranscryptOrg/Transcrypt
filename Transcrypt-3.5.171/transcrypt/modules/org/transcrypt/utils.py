...
class CommandArgs:		
	def parse (self):
		...
		self.argParser.add_argument ('-de', '--dextex', help = "debug with extended exception reports", action = 'store_true')
		self.argParser.add_argument ('-dm', '--dmap', help = "dump human readable source map", action = 'store_true')
		self.argParser.add_argument ('-dt', '--dtree', help = "dump syntax tree", action = 'store_true')
		...
		self.argParser.add_argument ('-t', '--tconv', help = "enable automatic conversion to truth value by default. DISADVISED, since it will result in a conversion for each boolean. Preferable use __pragma__ ('tconv') and __pragma__ (\'notconv\') to enable automatic conversion locally", action = 'store_true')
