...
import os
import os.path
import posixpath    # since the code uses '/' even on windows
import sys
import ast
import re
import copy
...
return list (selfFields).''' + comparatorName + '''(list (otherFields));
    ...
    def revisit_ImportFrom (self, importHoistMemo): # From ... import ... can import modules or facitities offered by modules
        ...
        # convert any relative imports to their full names
        modPath = node.module.split('.')
        if getattr(node, 'level', 0) > 0:
            myPath = self.module.name.split('.')
            modPath = myPath[:-1 * node.level] + modPath
        nodeModule = '.'.join(modPath)

        if nodeModule.startswith (self.stubsName):
            return
        ...
        try:
            ...
            if facilityImported:                                                        # At least one alias denoted a facility rather than a module
                module = self.useModule (nodeModule)                                   # Use module that contains it
                namePairs.append (utils.Any (name = alias.name, asName = alias.asname))
