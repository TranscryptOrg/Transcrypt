...
class CommandArgs:
    def parse (self):
        ...
        self.argParser.add_argument ('-im', '--imports', nargs='?', metavar='MODE', default='.compile', help = "specify how import statements are handled. Use `-im .compile` to compile all imported modules (the default). Use `-im .reference` for import statements with .js extensions (no compilation of referenced files). Use `-im .bundled` for import statements with original extensions (allows bundlers to handle imports and run transcrypt for each .py file).")
...
def enhanceException (exception, **kwargs):
    ...
    if isinstance (exception, Error):
        ...
        kwargs['message'] = '{}: {}'.format (kwargs.get('message', ''), str(exception))
        result = Error (**kwargs)
