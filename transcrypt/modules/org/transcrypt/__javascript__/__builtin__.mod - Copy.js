...
__pragma__ ('endif')
    ...
    function __n__ (keyed, key) {  //  Deal with negative indexes, key is abs value of a negative literal, rest would be too slow!
        if (keyed instanceof Array || typeof keyed == 'string') {
            // It's crucial that there are no enhanced JavaScript types based on Array's that should use the unaltered key
            return keyed [keyed.length - key];
        }
        else {
            // Javascript native object or Proxy, do not alter the key
            return keyed [key];
        }
    }
    
    function __nk__ (keyed, key) {  // Check existence of key c.q. deal with negative index literals, depending on type of 'keyed'
        if (keyed instanceof Array || typeof keyed == 'string') {
            // It's crucial that there are no enhanced JavaScript types based on Array's that should use the unaltered key
            return keyed [keyed.length - key];
        }
        else {
            return __k__ (keyed, key);
        }
    }
