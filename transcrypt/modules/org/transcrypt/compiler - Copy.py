    ...
    def visit_Subscript (self, node):
        if type (node.slice) == ast.Index:
            if type (node.slice.value) == ast.Tuple:    # Always overloaded, it must be an RHS index
                ...
                self.visit (node.slice.value)
                self.emit (')')
            else:                                       # It may be an LHS or RHS index, no operator overloading
                try:
                    isRhsIndex = not self.expectingNonOverloadedLhsIndex
                    self.expectingNonOverloadedLhsIndex = False
                    if isRhsIndex and self.allowKeyCheck:
                        # We're sure it's an RHS index and dict keychecking is on
                        # We may not be dealing with a dict here, but it's just an extra harmless check, apart from performance
                        if (
                            type (node.slice.value) == ast.UnaryOp and
                            type (node.slice.value.op == ast.USub) and
                            type (node.slice.value.operand) == ast.Num
                        ):
                            self.emit ('__nk__ (')
                            self.visit (node.value)
                            self.emit (', ')
                        ...
                        # No overloading, no keycheck
                        if (
                            type (node.slice.value) == ast.UnaryOp and
                            type (node.slice.value.op == ast.USub) and
                            type (node.slice.value.operand) == Num
                        ):
                            self.emit ('__n__ (')
                            self.visit (node.value)
                            self.emit (', ')
